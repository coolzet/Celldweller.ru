...
$idstr = 'iblog' . $id; // Присваиваем значению ID строчное значение и подставляем id, так как id не может быть числовым
$connect = mysqli_connect($hn, $un, $pw, $db);
$new = mysqli_query($connect, "SELECT id FROM `blog` ORDER BY `id` DESC"); // запрашиваем id с последней строки таблицы
...
$result_1 = mysqli_query($connect, "SELECT * FROM `blog` WHERE `id` = $rows;"); //Запрашиваем у БД запись с id, равным последней строке, в случае, если цикл проходит впервые
...
while ($istrue == 0){ // Пока PHP не найдет в БД хоть одну запись с таким id, будет повторяться этот цикл
	...
	$result_1 = mysqli_query($connect, "SELECT * FROM `blog` WHERE `id` = $rows;"); // Запрашиваем эту строку.
...
echo "<div id=\"$idstr\">" . "<div class=\"post_time\">Дата добавления: $postingtime</div>" . '<h2>'. $row['title'] . '</h2><br><div class="celltext">' . $row['text'] . "</div></div>";
